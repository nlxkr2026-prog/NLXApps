...
if uploaded_files:
    ...
    def calculate_xy_pitch(df):
        # 요청사항 1: Height 컬럼이 있는 파일로만 Pitch 계산
    ...
    if processed_list:
        ...
        st.subheader("📊 Summary Statistics (Shift X/Y & Pitch Included)")
        ...
        tab1, tab2, tab3 = st.tabs(["📏 Group A: 형상 & 간격", "🎯 Group B: 위치 편차", "🌐 3D View & Highlight"])
        ...
        with tab2:
            ...
            st.divider()
            v_file = st.selectbox("화살표 맵 파일 선택", p_df_b['File_Name'].unique())
            v_df = p_df_b[(p_df_b['File_Name'] == v_file) & p_df_b['Shift_X'].notna()]
            if not v_df.empty:
                ...
                fig_v.update_layout(height=700, yaxis=dict(scaleanchor="x", scaleratio=1), title=f"Vector Map: {v_file}")
                st.plotly_chart(fig_v, use_container_width=True)

        with tab3:
            st.header("🌐 Integrated 3D Structural View")
            # 3D 뷰 데이터 통합 로직 (Master 매칭 여부 자동 판단)
            if m_key != "Independent Analysis (No Master)":
            ...
            c_3d_met = st.selectbox("색상/하이라이트 지표", [c for c in ['Inferred_Layer', 'Radius', 'Height', 'Pitch_X', 'Pitch_Y', 'Shift_Norm'] if c in df3.columns])
            df3 = df3.dropna(subset=[c_3d_met])

            apply_th = st.checkbox("⚠️ Threshold Highlighting 적용 (Red/Yellow)", value=False)
            
            if apply_th:
                c_h, c_l = st.columns(2)
                h_th = c_h.number_input("High Threshold (Red Above)", value=float(df3[c_3d_met].max()))
                l_th = c_l.number_input("Low Threshold (Yellow Below)", value=float(df3[c_3d_met].min()))
            ...
            fig3.update_layout(scene=dict(aspectmode='data'), height=800, title=f"3D Map: {c_3d_met}")
