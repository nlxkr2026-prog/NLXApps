...
if uploaded_files:
    ...
    if all_data:
        ...
        with tab2:
            ...
            else: st.info("비교 분석을 위해 2개 이상의 레이어가 필요합니다.")
                    ...
                    if not base.empty:
                        for lyr in unique_layers:
                            ...
                            if not merged.empty:
                                ...
                                trend_list.append({'Source': src, 'Layer': lyr, 'Avg_DX': merged['DX'].mean(), 'Avg_DY': merged['DY'].mean(), 'Avg_Mag': merged['MAG'].mean(), 'Count': len(merged)})
