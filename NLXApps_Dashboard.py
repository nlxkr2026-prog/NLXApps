...
st.title("🔬 Bump Quality & Multi-Layer Master Analyzer")
...
if uploaded_files:
    ...
    master_key = st.selectbox("Master 파일 선택 (예: Height 또는 Multi-layer 파일)", list(raw_data.keys()))
    
    # Master 레이어링
    m_df = get_layer_info(raw_data[master_key], z_gap_threshold)
    layer_map = m_df[['Group_ID', 'Inferred_Layer']].drop_duplicates()
    ...
    for name, df in raw_data.items():
        if name == master_key:
            ...
            # 요청 반영: 마스터와 매칭되는 Group_ID만 유지 (Inner Join)
            if 'Group_ID' in df.columns:
                final_df = df.merge(layer_map, on='Group_ID', how='inner')
            else:
                st.warning(f"⚠️ '{name}' 파일에 Group_ID가 없어 분석에서 제외되었습니다.")
                continue
    ...
    if processed_list:
        ...
        # --- 상단 통계 대시보드 ---
        st.subheader("📊 Summary Statistics (Layer-Matched Data Only)")
        ...
        with t1:
            ...
            with col_a1:
                st.plotly_chart(px.box(full_df, x="File_Name", y=sel_met_a, color=c_grp, points=False, title=f"{sel_met_a} Boxplot"), use_container_width=True)
            with col_a2:
                # 히스토그램 가시성 해결
                b_mode = "overlay" if hist_layout == "Overlay (겹쳐보기)" else "group"
                f_col = "File_Name" if hist_layout == "Facet (파일별 분리 - 권장)" else None
                fig_h_a = px.histogram(full_df, x=sel_met_a, color="File_Name" if c_grp is None else c_grp, 
        ...
        with t2:
            ...
            with col_b1:
                st.plotly_chart(px.box(full_df, x="File_Name", y=sel_met_b, color=c_grp, points=False, title=f"{sel_met_b} Boxplot"), use_container_width=True)
            with col_b2:
                b_mode = "overlay" if hist_layout == "Overlay (겹쳐보기)" else "group"
                f_col = "File_Name" if hist_layout == "Facet (파일별 분리 - 권장)" else None
                fig_h_b = px.histogram(full_df, x=sel_met_b, color="File_Name" if c_grp is None else c_grp, 
                                       barmode=b_mode, facet_col=f_col, opacity=0.7, title=f"{sel_met_b} Distribution")
                st.plotly_chart(fig_h_b, use_container_width=True)
            
            st.divider()
            st.subheader("📍 Shift Vector Map (Inner Bump Shift)")
            v_file = st.selectbox("화살표 지도를 볼 파일 선택", full_df['File_Name'].unique())
            v_df = full_df[full_df['File_Name'] == v_file].dropna(subset=['Shift_X', 'Shift_Y'])
            if not v_df.empty:
        ...
        with t3:
            ...
            df3 = full_df[full_df['File_Name'] == t_3d]
            ...
            fig3 = px.scatter_3d(df3, x='Bump_Center_X' if 'Bump_Center_X' in df3.columns else 'X_Coord', 
                                 ...
                                 z=zc, color=c_3d, opacity=0.7)
            fig3.update_layout(scene=dict(aspectmode='data'))
            st.plotly_chart(fig3, use_container_width=True)
    else:
        st.error("❌ 매칭된 데이터가 없습니다. Group_ID가 일치하는지 확인하세요.")
else:
